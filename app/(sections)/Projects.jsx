...
const Projects = () => {
  ...
  return (
    <section id="projects" className="w-full bg-[var(--background)] relative overflow-hidden">
      {/* Background elements */}
      ...
      <div className="max-w-7xl mx-auto py-6 sm:py-12">
        {/* Section Header */}
        <motion.div
          className="relative px-3 sm:px-4 md:px-8"
        ...
        {/* Projects Container */}
        <div ref={containerRef} className=" sticky" style={{ height: `${projects.length * 100}vh` }}>
...
const ProjectCard = ({ project, index, scrollYProgress, totalProjects, hoveredProject, setHoveredProject }) => {
  ...
  const y = useTransform(progress, [0, 0.5], [1, -200])

  const scale = useTransform(progress, [0, 0.5, 1], [1, 1.05, 1])

  const imageScale = useTransform(cardScrollProgress.scrollYProgress, [0, 1], [1.2, 1])

  return (
    <div ref={cardRef} className="h-screen w-full flex items-center justify-center sticky top-0">
      <motion.div
        className="w-full border-[#01a9e3] border max-w-5xl mx-2 sm:mx-4 md:mx-8 rounded-3xl overflow-hidden shadow-xl"
        style={{
      ...
      >
        <div className="p-4 sm:p-6 md:p-8 lg:p-12">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#00bfff] text-center mb-4 sm:mb-8">{project.title}</h2>

          <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 lg:gap-12">
            {/* Description Side */}
            <div className="lg:w-2/5 flex flex-col justify-center">
              <div className="text-white/90 mb-4 sm:mb-6 text-base sm:text-lg">
                <p className="first-letter:text-3xl first-letter:font-bold first-letter:mr-1">{project.description}</p>
              </div>
            ...
            {/* Image Side */}
            <div className="lg:w-3/5 rounded-xl overflow-hidden relative h-48 sm:h-64 md:h-80 lg:h-96">
