      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
        ...
        >
          <div className="flex items-center">
            <svg
              className="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-[var(--primary)]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            ...
            <h1 className="font-bold text-lg sm:text-xl lg:text-2xl text-[var(--foreground)] tracking-wider">
          ...
          <ul className="flex items-center gap-6 xl:gap-10">
            {navLinks.map((link) => (
              <li key={link.name}>
                <a
                  href={link.href}
                  className="scroll-smooth p-2 text-[var(--foreground)] hover:text-[var(--primary)] transition-colors duration-200 font-medium relative group text-sm lg:text-base"
                >
                  {link.name}
              ...
              <ul className="space-y-4 sm:space-y-6">
                {navLinks.map((link) => (
                  <li key={link.name}>
                    <a
                      href={link.href}
                      className="text-[var(--foreground)] hover:text-[var(--primary)] transition-colors duration-200 text-base sm:text-lg font-medium block"
